var __reflect = (this && this.__reflect) || function (p, c, t) {
    p.__class__ = c, t ? t.push(c) : t = [c], p.__types__ = p.__types__ ? t.concat(p.__types__) : t;
};
var __extends = this && this.__extends || function __extends(t, e) { 
 function r() { 
 this.constructor = t;
}
for (var i in e) e.hasOwnProperty(i) && (t[i] = e[i]);
r.prototype = e.prototype, t.prototype = new r();
};
var NiuNiu;
(function (NiuNiu) {
    var GameHistoryRank = (function (_super) {
        __extends(GameHistoryRank, _super);
        function GameHistoryRank() {
            var _this = _super.call(this) || this;
            _this.group = new eui.Group();
            _this.scrollView = new eui.Scroller();
            _this.nums = 0;
            _this.addEventListener(egret.Event.ADDED_TO_STAGE, _this.onAddToStage, _this);
            return _this;
        }
        GameHistoryRank.prototype.onAddToStage = function (e) {
            this.removeEventListener(egret.Event.ADDED_TO_STAGE, this.onAddToStage, this);
            this.width = 321;
            this.height = 340;
            this.initRank();
        };
        GameHistoryRank.prototype.initRank = function () {
            var bgKuang = GameUtil.createBitmapByName("kuang2_png");
            this.addChild(bgKuang);
            bgKuang.width = this.width;
            bgKuang.height = this.height;
            var title = new egret.TextField();
            this.addChild(title);
            title.text = "战 绩";
            title.width = this.width;
            title.textAlign = egret.HorizontalAlign.CENTER;
            title.size = 25;
            title.bold = true;
            title.x = 15;
            title.textColor = 0x000000;
            this.scrollView = new eui.Scroller();
            this.addChild(this.scrollView);
            this.scrollView.width = 200;
            this.scrollView.height = 260;
            this.scrollView.x = 73;
            this.scrollView.y = 40;
            this.scrollView.$blendMode;
            this.group = new eui.Group();
            this.scrollView.viewport = this.group;
        };
        GameHistoryRank.prototype.addOne = function (cards1, cards2, isWin) {
            var playerCard1Type = GameUtil.getTypeByCard(cards1);
            var playerCard2Type = GameUtil.getTypeByCard(cards2);
            var playerCard1Name = GameUtil.getCardTypeNamebyType(playerCard1Type);
            var playerCard2Name = GameUtil.getCardTypeNamebyType(playerCard2Type);
            var str = "";
            if (isWin) {
                str = "庄:" + playerCard1Name + " vs 玩家:" + playerCard2Name + "   赢";
            }
            else {
                str = "庄:" + playerCard1Name + " vs 玩家:" + playerCard2Name + "   输";
            }
            var rect = new eui.Rect();
            rect.width = this.scrollView.width;
            if (isWin) {
                rect.fillColor = 0xff0000;
            }
            else {
                rect.fillColor = 0x00ff00;
            }
            rect.fillAlpha = 0.5;
            rect.height = 30;
            rect.y = 30 * this.nums + this.nums * 5;
            this.group.addChild(rect);
            var des = new egret.TextField();
            des.text = str;
            des.width = this.scrollView.width;
            des.textAlign = egret.HorizontalAlign.CENTER;
            des.size = 16;
            des.textColor = 0x000000;
            des.y = 30 * this.nums + this.nums * 5 + 10;
            this.group.addChild(des);
            console.log(this.scrollView.viewport.contentHeight);
            console.log(this.scrollView.viewport.height);
            //当排行榜超出框架,滚动到最下面
            if (this.scrollView.viewport.contentHeight > this.scrollView.viewport.height) {
                this.scrollView.viewport.scrollV = this.scrollView.viewport.contentHeight - this.scrollView.viewport.height + 35;
            }
            this.nums++;
        };
        return GameHistoryRank;
    }(egret.DisplayObjectContainer));
    NiuNiu.GameHistoryRank = GameHistoryRank;
    __reflect(GameHistoryRank.prototype, "NiuNiu.GameHistoryRank");
})(NiuNiu || (NiuNiu = {}));
//# sourceMappingURL=GameHistoryRank.js.map